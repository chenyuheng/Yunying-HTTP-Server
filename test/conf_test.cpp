...
TEST(ConfTest, FailLoading) {
  ...
  EXPECT_EQ(yunying::OriginType::StaticFile, yunying::Conf::getInstance().get_origin_type());
  EXPECT_EQ(".", yunying::Conf::getInstance().get_root_dir());
  EXPECT_EQ("cpc.people.com.cn", yunying::Conf::getInstance().get_upstream_host());
  EXPECT_EQ("157.185.156.194", yunying::Conf::getInstance().get_upstream_ip());
  EXPECT_EQ(80, yunying::Conf::getInstance().get_upstream_port());

  // load test_fail.lua and test values, should not change
  yunying::Conf::getInstance().set_lua_config_path("../test/config/test_fail.lua");
...
TEST(ConfTest, SuccessfulLoading) {
  ...
  yunying::Conf::getInstance().set_lua_config_path("../test/config/test_success.lua");
  yunying::Conf::getInstance().parseLua();
  ...
  EXPECT_EQ(yunying::OriginType::Upstream, yunying::Conf::getInstance().get_origin_type());

...
void HttpRequest::set_method(HttpMethod method) { method_ = method; }

void HttpRequest::set_path(const std::string path) { path_ = path; }
...
std::string HttpRequest::to_string() {
  ...
  request_raw += MethodString[method_] + " " + path_ + " " +
                 HttpVersionString[http_version_] + "\r\n";
...
void HttpResponse::delete_header(const std::string key) { headers_.erase(key); }

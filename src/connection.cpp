...
void Connection::recv() {
  ...
  if (received_raw_.find("\r\n\r\n") != std::string::npos) {
    std::string header =
        received_raw_.substr(0, received_raw_.find("\r\n\r\n"));
    ...
    if (header.find("CONTENT-LENGTH:") != std::string::npos) {
      std::string content_length_str =
          header.substr(header.find("CONTENT-LENGTH:") + 15);
      content_length_str =
          content_length_str.substr(0, content_length_str.find("\r\n"));
...
void Connection::send() {
  ...
  int n = ::send(fd_, send_raw_.c_str() + send_offset_,
                 send_raw_.size() - send_offset_, MSG_NOSIGNAL);
  if (n < 0) {
    fd_closed_ = true;

...
namespace yunying {
    void Conf::parseLua() {
        ...
        working_threads_num_ = luaGet<uint16_t>("working_thread_num", working_threads_num_);
        cache_size_bytes_ = luaGet<int>("cache_size_bytes", cache_size_bytes_);
        cache_default_max_age_ = luaGet<int>("cache_default_max_age", cache_default_max_age_);
        cache_clean_interval_ = luaGet<int>("cache_clean_interval", cache_clean_interval_);
    ...
    template <class T> T Conf::luaGet(std::string key, T default_val) {
